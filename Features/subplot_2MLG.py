...
sb = ['solid', '-.', '--']  # Different for e22[0] and e22[1]
...
for j in range(len(e22)):  # e22 → columns
    for i in range(len(w)):  # d → rows
        # Load data
        data_path = os.path.join(
            base_dir,
            'results/MLG_systems_0K/MLG_MLG/2spacer/data',
            f'ni1=0.45e12-d=6-w={w[i]}-e21=4-e22={e22[j]}.txt'
        )
        ...
        # Get subplot axes
        ax_ij = ax[i, j]
        ax_ij.plot(x, y1, ls=sb[0], color=colors[0], label='r0 = 0')
        ax_ij.plot(x, y2, ls=sb[1], color=colors[1], label='r0 = 5a0')
        ax_ij.plot(x, y3, ls=sb[2], color=colors[2], label='r0 = 10a0')

        # Title and labels
        ax_ij.set_title(f'w={w[i]}nm, e22={e22[j]}', fontsize=10)
        ax_ij.set_xlabel('n1')
...
save_path = os.path.join(
    ...
    'combined_e21=4_d=6_2MLG_subplots.pdf'
)
